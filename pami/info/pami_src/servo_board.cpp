...
void ServoBoardInit()
{
  ...
  ServoControllerInit(&servoCtrl_tst[0], SERVO_BOARD_ARM_ID, SERVO_BOARD_ARM_MIN, SERVO_BOARD_ARM_MAX, SERVO_BOARD_ARM_RETRACTED - 1.0, SERVO_BOARD_ARM_SPEED, SERVO_BOARD_ARM_ACCEL);
...
void ServoBoardSet(uint8_t servoId_u8, float servoAngle_f)
{
  ...
  if ( (servoAngle_f >= 0.0) && (servoAngle_f <= 180.0) )
  {
    ...
    if (servoBoardEnable_b_g == true)
    {
      ...
      servo.writeMicroseconds(pulselength_u16);
    }
    else
    {
